...
class Navbar extends Component {
	...
	delayedSearch = debounce((e) => {
		return this.props.fetchItems(e.target.value);
	}, 500);
	...
	render() {
		const showItem = this.props.currentUser.isAuthenticated ? (
			...
			<nav>
				<div className="nav-brand">
					<h1>
						...
						<Link to="/">
							{" "}
							<img src={logo} alt="hn-algolia" />
						</Link>
					</h1>
				</div>
				<div className="brand-name px-3">{showItem}</div>
				<div className="nav-input  mx-3">
					{this.props.currentUser.isAuthenticated && (
						<input
							...
							value={query}
							placeholder="Stories, polls, jobs, comment"
						/>
					)}
				</div>
				<div className=" nav-algolia ml-auto ">
					...
					>
						<span className="text-dark"> by </span>Algolia
					</a>
				</div>
